...
namespace trackr_api.Filters
{
    public class ActionLoggingFilter : IActionFilter
    {
        ...
        public void OnActionExecuting(ActionExecutingContext context)
        {
            ...
            _logger.LogInformation("Starting action from LogFilter: {Action} at {Time}", context.ActionDescriptor.DisplayName, DateTime.UtcNow);
        }
        ...
        public void OnActionExecuted(ActionExecutedContext context)
        {
            ...
            {
                _logger.LogError("Action {Action} failed with exceptionfrom LogFilter: {Exception} at {Time}",
                    context.ActionDescriptor.DisplayName, context.Exception.Message, DateTime.UtcNow);
            }
            else
            {
                _logger.LogInformation("Completed action from LogFilter: {Action} at {Time}", context.ActionDescriptor.DisplayName, DateTime.UtcNow);
            }
        }
    }
}
